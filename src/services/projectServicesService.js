...
export async function listService(projectId, page, itemsPerPage) {
  ...
  try {
    const response = await get(`${SERVICE_URL}`, {
      projectId,
      page,
...
export async function listProjectServicesByStatus(status, page, itemsPerPage) {
  const projectServices = []

  try {
    const response = await get(`${SERVICE_URL}/status`, {
      status,
      page,
      pageSize: itemsPerPage,
    })
    response?.data?.forEach((projectService) => {
