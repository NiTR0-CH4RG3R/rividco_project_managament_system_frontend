import { Box, IconButton, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Paper, TablePagination } from '@mui/material';
import { Add, Tab } from '@mui/icons-material';
import SearchBar from '../SearchBar/SearchBar';

export default function ListPage({
    columns = [
        { id: 'name', label: 'Dessert (100g serving)', minWidth: 170, align: 'left' },
    ...
    return (
        ...
        >
                            ...
                            <TableRow key={0}>
                                {
                                    columns.map((column) => (
                                        <TableCell
                                            ...
                                            style={{ ...((column.minWidth !== undefined) && { minWidth: column.minWidth }) }}
                                            sx={{ fontWeight: 'bold' }}
                                        >
                                            {column.label}
                                        </TableCell>
                        ...
                        <TableBody>
                            {
                                rows.map((row) => (
                                    <TableRow
                                        ...
                                        hover
                                        onClick={(event) => { return onRowClick(event, row.id); }}
                                        role="checkbox"
            ...
            >
                <IconButton
                    type='button'
                    sx={{ position: 'fixed', bottom: '5%', right: '2%', boxShadow: 3, backgroundColor: 'background.paper', color: 'text', p: 2 }}
